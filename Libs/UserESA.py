...
2021/02/10: Full-width judgment in Puckid and Sample Name
"""
import sys
import os
import math
...
import datetime
import xlrd
import unicodedata # by N.Mizuno at 2021/02/10
...
class UserESA:
    def __init__(self, fname=None, root_dir=".", beamline=None):
        ...
        if beamline.upper() == "BL32XU":
            ...
            self.confdir = os.environ['BLCONFIG']

            print("DIRDIR= %s" % self.confdir)
                    ...
                    for row in range(sheet.nrows):
                        ...
                        for col in range(sheet.ncols):
                            ...
                            if cell.ctype == xlrd.XL_CELL_NUMBER:
                                ...
                                # by N.Mizuno at 2021/02/10
                                if row > 2:
                                    if col == 0 or col == 2:
                                        for char in cell.value:
                                            check = unicodedata.east_asian_width(char)
                                            if check != "Na":
                                                is_error = True
                                                print("ERROR: Charcater Invalid.")
                                                print(cell.value)
                                # --- #
...
#                    self.cols.append(line)
    ...
    def makeCondList(self):
        ...
        for cols in self.contents:
            ...
            if self.beamline.lower() == "bl45xu" and distance < 140: distance = 140 # by N.Mizuno at 2020/10/13
            elif self.beamline.lower() == "bl41xu" and distance < 180: distance = 180 # by N.Mizuno at 2020/10/13
            hbeam, vbeam = self.checkBeamsize(beamsize)
            self.horbs = hbeam # by N.Mizuno at 2020/11/30
            self.verbs = vbeam # by N.Mizuno at 2020/11/30
...
#                dist_raster = 500.0
            ...
            line_str += "%7.5f,%f,%f,%f,%f,%f,%f,%f,%d,%d,%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%s,%f,%f,%f," \
                        "%f,%d,%d,%d,%d,%d" % \
                (wavelength,vbeam,hbeam,att_raster,hebi_att,exp_raster,dist_raster,
                loop_size,score_min,score_max,n_crystals,total_osc,osc_width,vbeam,hbeam,exp_ds,distance,dose_ds,0.0,1.0,1.0,
...
if __name__ == "__main__":
    ...
    # if sys.argv[2] is not None:
        beamline=sys.argv[2]
    u2db = UserESA(sys.argv[1], root_dir, beamline=beamline)
    u2db.makeCondList()
