...
class Zoo:
        ...
        if self.isConnect==False:
            ...
            for i in range(0,10):
                try:
                    ...
                    self.logger.info("Time out. Retry query.")
                    continue
        idx=0
        self.tray_list=[]
        for col in cols:
    ...
    def getCurrentPin(self):
        ...
        if recstr.rfind("fail") != -1:
            ...
            if int(error_code) == -1005000009:
                ...
                raise MyException("SPACE server does not know the current pin inforrmation")
    ...
    # isPE: Puck exchanger is used: True
    ...
    def waitSPACE(self):
        ...
        while (1):
            ...
            try:
                ...
                self.logger.debug("Query time out -> retry querying")
                continue
            # print "Received buffer in isBusy: %s"%recstr
            svoc_c = self.getSVOC_C(recstr)
            if svoc_c.rfind("ready") != -1:
    ...
    # if puck exchanger query -> pe=True
    def waitTillFinish(self, query_command):
        ...
        while (1):
            try:
                ...
                self.logger.info("Time out in query. Now retry will be executed.")
                continue
            self.logger.info("Received buffer in isBusy: %s"%recstr)
            svoc_c=self.getSVOC_C(recstr)
            if svoc_c.rfind("ready")!=-1:
    ...
    def getWavelength(self):
        ...
        while(1):
            try:
                self.bssr.sendall(com)
                recstr=self.bssr.recv(8000)
                self.logger.info("RECSTR=%s"%recstr)
            except:
                self.logger.info("Getting wavelength failed...Retry to get.")
                continue
            cols = recstr.split('/')
            #print cols
...
## Puck exchanger related codes ####
    #def pe_weitTillReady(self):
